...
// -----------------------------------------
//
// Source: PropertySearchDec21st.java
// Date  : 22 Dec 2021 21:08:23 ECT
 ...
 * Automatically generated load test program.
 */
public class PropertySearchDec21st extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:94.0) Gecko/20100101 Firefox/94.0";
	...
	private static final String EXTERNAL_PARAM_FILE = "PropertySearchDec21stExtParams.dat"; // input file name for very large parameter values
	...
	private static VarInputFileReader AddressEdited_csv = null;      // input file AddressEdited.csv, file scope: new line per loop
	...
	private String ScheduleNumber = null;                // var declaration from web admin var handler: scope = per loop
	private String SITUS_STR_NAME_PRE_DIR = null;        // var declaration from web admin var handler: scope = per loop
	private String SITUS_ADDR_NBR = null;                // var declaration from web admin var handler: scope = per loop
	private String SITUS_STR_NAME = null;                // var declaration from web admin var handler: scope = per loop
	private String SITUS_STR_NAME_POST_TYPE = null;      // var declaration from web admin var handler: scope = per loop
	private String Address = null;                       // var declaration from web admin var handler: scope = per loop
	
	/**
	 * constructor: called from load test plug-ins (scope = global).
	 */
	public PropertySearchDec21st()
	{
		super();
	}
	 ...
	 */
	public PropertySearchDec21st(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		if(isNonModularMode())
			ScheduleNumber = null;
		if(isNonModularMode())
			SITUS_STR_NAME_PRE_DIR = null;
		if(isNonModularMode())
			SITUS_ADDR_NBR = null;
		if(isNonModularMode())
			SITUS_STR_NAME = null;
		if(isNonModularMode())
			SITUS_STR_NAME_POST_TYPE = null;
		if(isNonModularMode())
			Address = "";
		log();
		log("<<< ScheduleNumber = " + ScheduleNumber);
			loopVarMap.put("ScheduleNumber", ScheduleNumber);
		log("<<< SITUS_STR_NAME_PRE_DIR = " + SITUS_STR_NAME_PRE_DIR);
			loopVarMap.put("SITUS_STR_NAME_PRE_DIR", SITUS_STR_NAME_PRE_DIR);
		log("<<< SITUS_ADDR_NBR = " + SITUS_ADDR_NBR);
			loopVarMap.put("SITUS_ADDR_NBR", SITUS_ADDR_NBR);
		log("<<< SITUS_STR_NAME = " + SITUS_STR_NAME);
			loopVarMap.put("SITUS_STR_NAME", SITUS_STR_NAME);
		log("<<< SITUS_STR_NAME_POST_TYPE = " + SITUS_STR_NAME_POST_TYPE);
			loopVarMap.put("SITUS_STR_NAME_POST_TYPE", SITUS_STR_NAME_POST_TYPE);
		log("<<< Address = " + Address);
			loopVarMap.put("Address", Address);
		...
		try
		{
			...
			{
				if (!AddressEdited_csv.getNextLine())
				{
					abortByEofInputFile();
					return true;		// eof of input file - no error from web server
				}
				SITUS_ADDR_NBR = new String(AddressEdited_csv.getLineVar(2));
				SITUS_STR_NAME = new String(AddressEdited_csv.getLineVar(4));
				SITUS_STR_NAME_POST_TYPE = new String(AddressEdited_csv.getLineVar(5));
				SITUS_STR_NAME_PRE_DIR = new String(AddressEdited_csv.getLineVar(3));
			}
			log();
			log("<<< SITUS_ADDR_NBR = " + SITUS_ADDR_NBR);
			loopVarMap.put("SITUS_ADDR_NBR", SITUS_ADDR_NBR);
		...
		log("# Page #2: search");
		log("# ---------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #2: search", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
		...
		log("# Page #3: Details");
		log("# ----------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #3: Details", 3000, 35, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
		pageThreadHandler = new HttpTestURLThreadHandler(threadStep - 1, maxParallelThreadsPerUser, performanceData, this);   // support for parallel processing of http(s) requests within a page. hint: param #2 is the number of parallel threads per user
		log();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 41298
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246402L;
		...
		String requestFile0002 = "/static/CCD.PublicAppSkin/1.1.11/css/publicappskin.min.css";
		...
		String requestFile0003 = "/static/CCD.Bootstrap/3.2.10/css/bootstrap.min.css";
		...
		String requestFile0004 = "/property/Scripts/jquery-1.9.1.min.js";
		...
		String requestFile0005 = "/property/Scripts/jquery.MultiFile.pack.js";
		...
		String requestFile0006 = "/static/CCD.FontAwesome/4.2.1/css/fontawesome.min.css";
		...
		String requestFile0007 = "/property/Scripts/jquery-ui-1.9.2.js";
		...
		String requestFile0008 = "/property/Scripts/jquery.watermark.min.js";
		...
		// --- HTTP REQUEST: Test [9] <- WEB ADMIN Index 13 ---
		String requestProt0009 = "https";
		String requestHost0009 = "qa.denvergov.org";
		int    requestPort0009 = 443;
		String requestFile0009 = "/property/Scripts/jquery.tipTip.minified.js";
		...
		String requestFile0010 = "/property/Scripts/jquery.history.js";
		...
		String requestFile0011 = "/property/Scripts/jquery.tablesorter.js";
		...
		String requestFile0012 = "/property/Scripts/app/ko.protected.js";
		...
		String requestFile0013 = "/property/Scripts/knockout-2.1.0.debug.js";
		...
		String requestFile0014 = "/property/Scripts/app/ko.dialog.js";
		...
		String requestFile0015 = "/property/Scripts/jquery.maskedinput-1.3.1.min.js";
		...
		String requestFile0016 = "/property/Scripts/knockout.validation.js";
		...
		String requestFile0017 = "/property/Scripts/moment.js";
		...
		String requestFile0018 = "/property/Scripts/jquery.maskMoney.js";
		...
		String requestFile0019 = "/property/Scripts/Analytics/DenverApps.Analytics.js";
		...
		String requestFile0020 = "/property/Content/Pager.css";
	...
	boolean executePage_1_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestFile0021 = "/property/Scripts/jquery.pager.js";
		...
		String requestFile0022 = "/property/Content/mats_styles.css";
		...
		String requestFile0024 = "/property/Scripts/jquery.tmpl.min.js";
		...
		String requestFile0025 = "/property/Content/themes/redmond/jquery-ui-1.8.16.custom.css";
		...
		String requestFile0027 = "/property/Content/responsive.css";
		...
		// --- HTTP REQUEST: Test [28] <- WEB ADMIN Index 38 ---
		log();
		log("# title: 404 Error | City and County of Denver");
		String requestProt0028 = "https";
		String requestHost0028 = "qa.denvergov.org";
		int    requestPort0028 = 443;
		String requestFile0028 = "/favicon.ico";
		...
		// --- HTTP REQUEST: Test [29] <- WEB ADMIN Index 39 ---
		...
		// --- HTTP REQUEST: Test [31] <- WEB ADMIN Index 43 ---
		String requestProt0031 = "https";
		String requestHost0031 = "qa.denvergov.org";
		int    requestPort0031 = 443;
		String requestFile0031 = "/maps/content/js/JQueryPlugins.js";
		...
		String requestFile0032 = "/maps/Content/OpenLayers/OpenLayers.js";
		...
		// --- HTTP REQUEST: Test [34] <- WEB ADMIN Index 47 ---
		...
		// --- HTTP REQUEST: Test [35] <- WEB ADMIN Index 48 ---
		String requestProt0035 = "https";
		String requestHost0035 = "qa.denvergov.org";
		int    requestPort0035 = 443;
		String requestFile0035 = "/maps/content/openlayers/theme/default/style.css";
		...
		String requestFile0036 = "/maps/content/TipTip/tipTip.css";
	...
	boolean executePage_2(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [38] <- WEB ADMIN Index 57 ---
		// execute inline script "ConcatenateAddress"
		inlineScriptContext = executeInlineScript_1638912870869(threadStep, totalLoopCounter, innerLoopContext);
		...
		String requestFile0038 = "/denvermapsservices/FindAutoSuggestResults/" +
				"?callback=jQuery19103858152020937886_1638474246699" +
				"&stem=211%20n%20king" +
				"&types=BusinessPersonalPropertySiteaddress,BusinessPersonalPropertySchednum,BusinessPersonalPropertyPin,BusinessPersonalPropertyOwner,BusinessPersonalPropertyCoOwner" +
				"&_=1638474246702";
		requestFile0038 = Lib.replaceTextPattern(requestFile0038, "211%20n%20king", Address, 0);		// search and replace request file fragment "211%20n%20king" with variable 'Address' on all occurrences
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 141
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, null);		// Test [38] <- Index 38
		if(replayMode)
		{
			long proxyDataRecordId =1638474266989L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [39] <- WEB ADMIN Index 59 ---
		String requestProt0039 = "https";
		String requestHost0039 = "qa.denvergov.org";
		int    requestPort0039 = 443;
		String requestFile0039 = "/property/businesspersonalproperty/search/bppsearch/0/" +
				"?searchText=211%20n%20king%20st%20" +
				"&page=1";
		requestFile0039 = Lib.replaceTextPattern(requestFile0039, "211%20n%20king%20st%20", Address, 0);		// search and replace request file fragment "211%20n%20king%20st%20" with variable 'Address' on all occurrences
		...
		// verify response: status code = 200, content type = "APPLICATION/JSON", header text fragment = [verification disabled], recorded content size = 315
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, null);		// Test [39] <- Index 39
		if(replayMode)
		{
			long proxyDataRecordId =1638474268483L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [40] <- WEB ADMIN Index 62 ---
		String requestProt0040 = "https";
		String requestHost0040 = "qa.denvergov.org";
		int    requestPort0040 = 443;
		String requestFile0040 = "/property/businesspersonalproperty/search/bppsearch/0/" +
				"?searchText=211%20n%20king%20st%20" +
				"&page=1";
		requestFile0040 = Lib.replaceTextPattern(requestFile0040, "211%20n%20king%20st%20", Address, 0);		// search and replace request file fragment "211%20n%20king%20st%20" with variable 'Address' on all occurrences
		...
		String requestHeader0040 = "GET " + requestFile0040 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"X-Requested-With: XMLHttpRequest\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Referer: https://qa.denvergov.org/property?q=211%20n%20king%20st%20\r\n" + 
				"Sec-Fetch-Dest: empty\r\n" + 
				"Sec-Fetch-Mode: cors\r\n" + 
				"Sec-Fetch-Site: same-origin\r\n" + 
				"\r\n";
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474270698L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		try
		{
			// extract var 'ScheduleNumber'
			ScheduleNumber = jsonHtmlBasedExtractor.getNodeValue("$.Properties[0].ScheduleNumber");		// recorded value = 0508325017000
			log("<<< ScheduleNumber = " + ScheduleNumber);
			loopVarMap.put("ScheduleNumber", ScheduleNumber);
			// Replace the value of a variable with a user given value in case of a blank text extracted during the load test
			if (false && ScheduleNumber!= null && ScheduleNumber.trim().length() == 0)
			{
				ScheduleNumber = "";
				log("runtime note: the extraction of a new value for the variable 'ScheduleNumber' ends up with an empty string - a substitute value was set");
				log("<<< ScheduleNumber = " + ScheduleNumber);
				loopVarMap.put("ScheduleNumber", ScheduleNumber);
			}
		}
		catch (Exception e) { ScheduleNumber = null; log(e); }
		if (ScheduleNumber == null)
		{
			// failure - dump wrong response content and abort current outer loop - after that start next loop
			String errorText = "*** error: unable to extract var 'ScheduleNumber' from json response data";
			log(errorText);
			terminateFailedUser(testURL);		// set the user to be terminated at end of loop ? - only performed if the URL call has marked before to support that !
			threadStep = performanceData.setFailed(threadStep, HttpTestURL.STATUS_TYPE_USER_SPECIFIC_TEST_FAILED, errorText, testURL, this);
	...
	boolean executePage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		// --- HTTP REQUEST: Test [42] <- WEB ADMIN Index 67 ---
		log();
		log("# title: | 0508325017000");
		String requestProt0042 = "https";
		String requestHost0042 = "qa.denvergov.org";
		int    requestPort0042 = 443;
		String requestFile0042 = "/property/businesspersonalproperty/assessment/0508325017000/";
		requestFile0042 = Lib.replaceTextPattern(requestFile0042, "0508325017000", ScheduleNumber, 0);		// search and replace request file fragment "0508325017000" with variable 'ScheduleNumber' on all occurrences
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 33621
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, null);		// Test [42] <- Index 42
		if(replayMode)
		{
			long proxyDataRecordId =1638474287063L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		// --- HTTP REQUEST: Test [43] <- WEB ADMIN Index 68 ---
		String requestProt0043 = "https";
		String requestHost0043 = "qa.denvergov.org";
		int    requestPort0043 = 443;
		String requestFile0043 = "/property/Scripts/jquery-1.7.1.min.js";
		String requestHeader0043 = "GET " + requestFile0043 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Referer: https://qa.denvergov.org/property/businesspersonalproperty/assessment/0508325017000/\r\n" + 
				"Sec-Fetch-Dest: script\r\n" + 
				"Sec-Fetch-Mode: no-cors\r\n" + 
				"Sec-Fetch-Site: same-origin\r\n" + 
				"\r\n";
		...
		testURL.replaceRequestHeaderFieldTextPattern("0508325017000", ScheduleNumber);		// search within all request header fields a text pattern and replace it with a variable value
		if (userNextProxyConfig != null)
			testURL.setProxy(userNextProxyConfig);
		testURL.setDNSContext(userDNSContext);
		testURL.setFailureActionType(HttpTestURL.FAILURE_ACTION_IGNORE_ERROR);
	...
	boolean synchResponsesParallelRequestsPage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246574L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246591L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246598L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246614L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246621L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246624L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246628L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246664L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246677L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246680L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246686L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246692L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246693L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246705L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246714L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246732L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246758L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
	...
	private boolean writeHttpAsyncResponseTabOutput_1 (LoadtestPluginContext loopPluginContext, int lastThreadStepInMainThread, InnerLoopContext innerLoopContext, int totalLoopCounter) throws Exception
	{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246760L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246770L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246787L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246789L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246790L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246791L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246799L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246800L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246817L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		// verify response: status code = 404, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 16660
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {404}, null, null, null);		// Test [28] <- Index 28
		if(replayMode)
		{
			long proxyDataRecordId =1638474246898L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		// verify response: status code = 200, content type = "APPLICATION/X-JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 176440
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/X-JAVASCRIPT", null, null);		// Test [29] <- Index 29
		if(replayMode)
		{
			long proxyDataRecordId =1638474246916L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474246926L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474247010L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474247015L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474247054L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474247061L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474247063L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474247075L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
	...
	boolean synchResponsesParallelRequestsPage_3(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext, Object httpLogVectorObject) throws Exception
	{
		...
		// verify response: status code = 200, content type = "APPLICATION/JAVASCRIPT", header text fragment = [verification disabled], recorded content size = 112288
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JAVASCRIPT", null, null);		// Test [43] <- Index 43
		if(replayMode)
		{
			long proxyDataRecordId =1638474287176L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
	...
	private boolean writeHttpAsyncResponseTabOutput_3 (LoadtestPluginContext loopPluginContext, int lastThreadStepInMainThread, InnerLoopContext innerLoopContext, int totalLoopCounter) throws Exception
	{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1638474287252L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("PropertySearchDec21st", concurrentUsers, ParseArgs.getString(args, "-executionPlan"));
		...
		performanceData.setInfoText("PropertySearchDec21st");
		...
		try
		{
			AddressEdited_csv = new VarInputFileReader(getSymmetricEncryptContext(), "AddressEdited.csv", "#", ",", AbstractInputFileReader.EOF_REOPEN_FILE, prxCharEncoding);
			AddressEdited_csv.setTrimValues(true);
		}
		catch (IOException ie)
		{
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new PropertySearchDec21st());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				PropertySearchDec21st simulatedUser = new PropertySearchDec21st(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "ConcatenateAddress"
	public static String getInlineScriptCode_1638912870869()
	{
		return "Address=SITUS_ADDR_NBR+\" \"+SITUS_STR_NAME_PRE_DIR+\" \"+SITUS_STR_NAME+\" \"+SITUS_STR_NAME_POST_TYPE\r\n\r\nAddress=strReplace(Address,\"\\ \",\"%20\")\r\n\r\nPrint Address";
	}
	
	
	/**
	* Execute the inline script "ConcatenateAddress"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1638912870869(int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("ConcatenateAddress", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_LOOP_ITEM_START, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler);
		inlineScriptContext.setThreadStep(threadStep);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1638912870869(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("SITUS_ADDR_NBR", Lib.nullToBlank(SITUS_ADDR_NBR), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar1);
		LoadtestInlineScriptVar inputVar2 = new LoadtestInlineScriptVar("SITUS_STR_NAME", Lib.nullToBlank(SITUS_STR_NAME), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar2);
		LoadtestInlineScriptVar inputVar3 = new LoadtestInlineScriptVar("SITUS_STR_NAME_POST_TYPE", Lib.nullToBlank(SITUS_STR_NAME_POST_TYPE), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar3);
		LoadtestInlineScriptVar inputVar4 = new LoadtestInlineScriptVar("SITUS_STR_NAME_PRE_DIR", Lib.nullToBlank(SITUS_STR_NAME_PRE_DIR), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addInputVar(inputVar4);
		LoadtestInlineScriptVar outputVar1 = new LoadtestInlineScriptVar("Address", Lib.nullToBlank(Address), 1);		// note: parameter no. 3 is the scope of the var
		inlineScriptContext.addOutputVar(outputVar1);
		inlineScriptExecutor.execute();		// execute inline script
		Address = inlineScriptContext.getOutputVar("Address").getVarValue();
		log("<<< Address = " + Address);
		loopVarMap.put("ScheduleNumber", ScheduleNumber);
		loopVarMap.put("SITUS_STR_NAME_PRE_DIR", SITUS_STR_NAME_PRE_DIR);
		loopVarMap.put("SITUS_ADDR_NBR", SITUS_ADDR_NBR);
