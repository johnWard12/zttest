 ...
 * Automatically generated load test program.
 */
public class sharepoint extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:93.0) Gecko/20100101 Firefox/93.0";
	...
	// --- embedded NTLM authentication ---
	private static NtlmConfig ntlmConfig = new NtlmConfig(2, "ABCLLC", "spobserver", "DcCcwKzpK7hN2BQ8EsdN", "", true);
	...
	private static final String EXTERNAL_PARAM_FILE = "sharepointExtParams.dat";            // input file name for very large parameter values
	 ...
	 */
	public sharepoint(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		testURL.setNtlmAuthentication(ntlmConfig);
		...
		// verify response: status code = 302, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 196
		...
		if(replayMode)
		{
			long proxyDataRecordId =1641405044542L;
		...
		String requestHost0002 = "sharepoint.abcsys.com";
		int    requestPort0002 = 443;
		String requestFile0002 = "/_layouts/15/start.aspx";
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 7348
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "span id=\"DeltaPlaceHolderPageTitleInTitleArea\"");		// Test [2] <- Index 2
		if(replayMode)
		{
			long proxyDataRecordId =1641405044957L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		String requestHost0003 = "sharepoint.abcsys.com";
		int    requestPort0003 = 443;
		String requestFile0003 = "/SitePages/Home.aspx" +
				"?AjaxDelta=1" +
				"&isStartPlt1=1641405045035";
		String requestHeader0003 = "GET " + requestFile0003 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				...
				"Accept-Language: en-US,en;q=0.5\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Pragma: SharePointAjaxDelta=|:|_CATALOGS|MASTERPAGE|SEATTLE.MASTER:10.15.0.0.0.15.0.5399.1000.9.FALSE.:en-US:en-US:RW\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Referer: https://sharepoint.abcsys.com/_layouts/15/start.aspx\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/PLAIN", header text fragment = [verification disabled], recorded content size = 149
		// content test algorithm: check size +/- 5%, defined content size = 149
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/PLAIN", null, 149, 5);		// Test [3] <- Index 3
		if(replayMode)
		{
			long proxyDataRecordId =1641405045200L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		log();
		log("# title: ABC LLC Home Site - Home");
		String requestProt0004 = "https";
		String requestHost0004 = "sharepoint.abcsys.com";
		int    requestPort0004 = 443;
		String requestFile0004 = "/SitePages/Home.aspx";
		...
		// verify response: status code = 200, content type = [verification disabled], header text fragment = [verification disabled], recorded content size = 67193
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, null, null, "ABC LLC Home Site");		// Test [4] <- Index 4
		if(replayMode)
		{
			long proxyDataRecordId =1641405045764L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("sharepoint", concurrentUsers, ParseArgs.getString(args, "-executionPlan"));
		...
		performanceData.setInfoText("sharepoint");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new sharepoint());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				sharepoint simulatedUser = new sharepoint(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
