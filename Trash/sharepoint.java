...
public class sharepoint extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36";
	...
	private static NtlmConfig ntlmConfig = new NtlmConfig(2, "ABCLLC", "spobserver@abcsys.local", "DcCcwKzpK7hN2BQ8EsdN", "", true);
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		if(replayMode)
		{
			long proxyDataRecordId =1641400722654L;
		...
		String requestFile0002 = "/_layouts/15/start.aspx#/SitePages/Home.aspx";
		...
		// content test algorithm: none - no content check
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {400}, null, null, null);		// Test [2] <- Index 2
		if(replayMode)
		{
			long proxyDataRecordId =1641400722655L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
