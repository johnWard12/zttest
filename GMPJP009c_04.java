...
// -----------------------------------------
//
// Source: GMPJP009c_04.java
// Date  : 24 May 2021 23:17:20 ECT
 ...
 * Automatically generated load test program.
 */
public class GMPJP009c_04 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	public static final String prxVersion = "V5.5-Z";
	public static final int    prxCharEncoding = 3;                         // 1 = OS Default, 2 = ISO-8859-1, 3 = UTF-8
	public static final String testDescription = "with inline script";
	...
	public static final String USER_INPUT_FIELD_TestString = "AQAKVGVzdFN0cmluZwEAClRlc3RTdHJpbmcBACt0YXJnZXQuanAud3d3Lm1ldGxpZmUuY29tL3Byb2R1Y3RzL2FubnVpdHkv";		// internal object-string representation of user input field 'TestString'
	...
	private static final String EXTERNAL_PARAM_FILE = "GMPJP009c_04ExtParams.dat";          // input file name for very large parameter values
	...
	private static String TestString = null;                                     // var declaration from web admin var handler: scope = global
	 ...
	 */
	public GMPJP009c_04(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	private boolean execute(int totalLoopCounter) throws Exception
	{
		...
		log();
		log();
		log("# Page #1: InvokeUrl");
		log("# ------------------");
		threadStep = setPageBreak(performanceData, threadStep, "Page #1: InvokeUrl", 0, 0, -1);		// hint: param #4 is the user's think time in milliseconds, param #5 is randomness of the user's think time in percent (+/- 0..100%), param #6 is the maximum acceptable response time in milliseconds (-1 = not configured)
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "target.jp.www.metlife.com";
		int    requestPort0001 = 443;
		String requestFile0001 = "/products/annuity/";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept-Language: en\r\n" + 
				"Accept: */*\r\n" + 
				"Accept-Encoding: gzip, deflate\r\n" + 
				"Host: target.jp.www.metlife.com\r\n" + 
		...
		// verify response: status code = 200, content type = "TEXT/HTML", header text fragment = [verification disabled], recorded content size = 15470
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "TEXT/HTML", null, "link rel=\"canonical\" href=\"https://target.jp.www.metlife.com/products/annuity/\"/");		// Test [1] <- Index 1
		if(replayMode)
		{
			long proxyDataRecordId =1620854940097L;
		...
		// execute inline script "TestString"
		inlineScriptContext = executeInlineScript_1621881487918(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
	...
	public static void main(String[] args)
	{
		...
		if ((ParseArgs.hasOption(args, "-h")) || (ParseArgs.hasOption(args, "-help")))
		{
			...
			System.out.println();
			System.out.println("-TestString <user-input-field>           ->> required argument: TestString");
			System.out.println();
		...
		// parse -TestString argument or ask --> <user-input-field>: proposed value = "target.jp.www.metlife.com/products/annuity/"
		try
		{
			TestString = UserInputField.readFromObjectString(USER_INPUT_FIELD_TestString).readUserInputValue(args);
			if (TestString.startsWith("\"") && TestString.endsWith("\""))
				TestString = TestString.substring(1, TestString.length() - 1);
		}
		catch (IOException ie)
		{
			System.out.println("*** INTERNAL ERROR: invalid user input field object-string declaration for 'TestString' ***");
			ie.printStackTrace();
			System.exit(-2);
		}
		...
		String genericFileName = PerformanceData.proposeFileName("GMPJP009c_04", concurrentUsers, ParseArgs.getString(args, "-executionPlan"));
		...
		performanceData.setInfoText("GMPJP009c_04");
		...
		performanceData.addUserInputField("TestString", TestString);		// add user input field name and value to performance data
		...
		{
			System.out.println("global var <<< TestString = " + TestString);
			System.out.println();
		}
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new GMPJP009c_04());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				GMPJP009c_04 simulatedUser = new GMPJP009c_04(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "TestString"
	public static String getInlineScriptCode_1621881487918()
	{
		return "IF !strContains(getHTTPResponseContent(), TestString) THEN\r\nabort(\"TestString \" + TestString + \" not found. below is the Response content: \" + getHTTPResponseContent())\r\nENDIF\r\n";
	}
	
	
	/**
	* Execute the inline script "TestString"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1621881487918(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("TestString", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1621881487918(), inlineScriptContext);
		log("Executing inline script \"" + inlineScriptContext.getScriptTitle() + "\"");
		LoadtestInlineScriptVar inputVar1 = new LoadtestInlineScriptVar("TestString", Lib.nullToBlank(TestString), 3);		// note: parameter no. 3 is the scope of the var
