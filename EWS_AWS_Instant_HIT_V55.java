...
// -----------------------------------------
//
// Source: EWS_AWS_Instant_HIT_V55.java
// Date  : 12 Aug 2021 20:29:04 ECT
 ...
 * Automatically generated load test program.
 */
public class EWS_AWS_Instant_HIT_V55 extends HttpLoadTest implements Runnable, ThreadStepInterface, SetThreadStepInterface, SSLSessionCacheStatisticInterface, VaryingLoadInterface, VaryingTestDurationInterface, SuspendResumeInterface, GetRealTimeUserInputFieldsInterface
{
	...
	public static String USER_AGENT_1 = "PostmanRuntime/7.26.10";
	...
	private static final String EXTERNAL_PARAM_FILE = "EWS_AWS_Instant_HIT_V55ExtParams.dat";// input file name for very large parameter values
	 ...
	 */
	public EWS_AWS_Instant_HIT_V55()
	{
		super();
	}
	 ...
	 */
	public EWS_AWS_Instant_HIT_V55(int maxLoops, int requestTimeout, int threadNumber)
	{
		super();
	...
	boolean executePage_1(int totalLoopCounter, LoadtestPluginContext loopPluginContext, InnerLoopContext innerLoopContext,int innerLoopCounter, int totalInnerLoopCount, Object httpLogVectorObject) throws Exception
	{
		...
		String requestHost0001 = "iws-can-prd.gcp.efx";
		int    requestPort0001 = 443;
		String requestFile0001 = "/verifier-service/api/status";
		String requestHeader0001 = "GET " + requestFile0001 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: */*\r\n" + 
				"Postman-Token: 8a19a3fb-484a-49e5-adc8-5c3fb548f1f4\r\n" + 
				"Host: iws-can-prd.gcp.efx\r\n" + 
		...
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/JSON", null, "\"status\":\"up\"");		// Test [1] <- Index 1
		if(replayMode)
		{
			long proxyDataRecordId =1627075753599L;
		...
		String requestHost0002 = "iws-can-prd.gcp.efx";
		int    requestPort0002 = 443;
		String requestFile0002 = "/verifier-service/api/report";
		String requestContent0002 = 
				"{" + "\r\n" + 
				"  \"personalInformation\": {" + "\r\n" + 
				"    \"address\": {" + "\r\n" + 
				"      \"addressLine1\": \"101 KING ST\"," + "\r\n" + 
				"      \"addressLine2\": \"\"," + "\r\n" + 
				"      \"city\": \"Toronto\"," + "\r\n" + 
				"      \"postalCode\": \"M1J3P5\"," + "\r\n" + 
				"      \"province\": \"ON\"" + "\r\n" + 
				...
				"      \"middleName\": \"\"" + "\r\n" + 
				"    }," + "\r\n" + 
				"    \"socialInsuranceNumber\": \"491215737\"" + "\r\n" + 
				"  }," + "\r\n" + 
				"    \"additionalInformation\": {" + "\r\n" + 
				"    \"customerReferenceNumber\": \"apicaTestRunfromVE\",    " + "\r\n" + 
				"    \"memberNumber\": \"001PZ41934\"    " + "\r\n" + 
				"  }" + "\r\n" + 
				"}";
		String requestHeader0002 = "POST " + requestFile0002 + " HTTP/" + httpProtocolVersion + "\r\n" + 
				"Content-Type: application/vnd.equifax.iws.api.v2+json\r\n" + 
				"channel: sts\r\n" + 
				"applicationid: apigee\r\n" + 
				"Authorization: Basic dmVyaWZpZXI6dmVyaWZpZXI=\r\n" + 
				"User-Agent: " + USER_AGENT_1 + "\r\n" + 
				"Accept: */*\r\n" + 
				"Postman-Token: dd5c0906-7863-4612-a5de-1ae965510afe\r\n" + 
				"Host: iws-can-prd.gcp.efx\r\n" + 
				"Accept-Encoding: gzip, deflate, br\r\n" + 
				"Connection: Keep-Alive\r\n" + 
				"Content-Length: " + requestContent0002.getBytes().length + "\r\n" + 		// Content-Length: 527
				"\r\n";
		...
		// verify response: status code = 200, content type = "APPLICATION/VND.EQUIFAX.IWS.API.V2+JSON", header text fragment = [verification disabled], recorded content size = 4535
		// content test algorithm: search text
		urlCallPassed = httpResponseOk(testURL, threadStep, new int[] {200}, "APPLICATION/VND.EQUIFAX.IWS.API.V2+JSON", null, "\"code\":200", "OR", "\"status\":\"completed\"");		// Test [2] <- Index 2
		if(replayMode)
		{
			long proxyDataRecordId =1627075768179L;
			createReplaySnapShotRecord(threadLoopCounter, proxyDataRecordId, testURL, replaySnapShotsDump);
		}
		if (!urlCallPassed)
		{
		...
		// execute inline script "Instant_SRE_nohit-Assertion"
		inlineScriptContext = executeInlineScript_1627076347791(testURL, urlCallPassed, threadStep - 1, totalLoopCounter, innerLoopContext);
	...
	public static void main(String[] args)
	{
		...
		String genericFileName = PerformanceData.proposeFileName("EWS_AWS_Instant_HIT_V55", concurrentUsers, ParseArgs.getString(args, "-executionPlan"));
		...
		performanceData.setInfoText("EWS_AWS_Instant_HIT_V55");
		...
		LoadtestPluginContext globalPluginContext = new LoadtestPluginContext(prxVersion, prxCharEncoding, new EWS_AWS_Instant_HIT_V55());
		...
		if (hasExecutionPlan())
		{
			...
			try
			{
				...
				EWS_AWS_Instant_HIT_V55 simulatedUser = new EWS_AWS_Instant_HIT_V55(maxPlannedLoops, plannedRequestTimeout, getUserContextTable().getUserContextList().size());
	...
	// source code of inline script "Instant_SRE_nohit-Assertion"
	public static String getInlineScriptCode_1627076347791()
	{
		return "resp = getHTTPResponseContent();\r\n\r\ninstantSuccessPath = \"verificationResponse.success\";\r\nsreSuccessPath = \"unverifiedResponse.success\";\r\ninstantHitPath = \"verificationResponse.hit\";\r\nsreHitPath = \"unverifiedResponse.hit\";\r\n\r\ninstantSuccess = strToLower(jsonGetData(resp, instantSuccessPath));\r\nsreSuccess = strToLower(jsonGetData(resp, sreSuccessPath));\r\ninstantHit = strToLower(jsonGetData(resp, instantHitPath));\r\nsreHit = strToLower(jsonGetData(resp, sreHitPath));\r\n\r\n\r\nif((instantSuccess != \"true\") or (sreHit != \"true\") or (instantHit != \"true\")) then\r\n    abort(\"The response did not contain one or both of the following in the response, instant response success is: \"+instantSuccess +\" and sre response success is: \"+sreSuccess+\" and instant response hit is: \"+instantHit +\" and sre response hit is: \"+sreHit+\" when it should have been true for all of them.\")\r\nendif\r\n";
	}
	
	
	/**
	* Execute the inline script "Instant_SRE_nohit-Assertion"
	* 
	...
	*/
	public LoadtestInlineScriptContext executeInlineScript_1627076347791(HttpTestURL testURL, boolean urlCallPassed, int threadStep, int totalLoopCounter, InnerLoopContext innerLoopContext)
	{
		LoadtestInlineScriptContext inlineScriptContext = new LoadtestInlineScriptContext("Instant_SRE_nohit-Assertion", ProxySnifferVarSourceInlineScript.EXEC_SCOPE_URL_END, "", this, LoadtestInlineScriptContext.RESULT_TYPE_SET_OUTPUT_VARS, 15, threadNumber, threadLoopCounter, socketPool, cookieHandler, testURL, threadStep);
		inlineScriptContext.setUrlCallPassed(urlCallPassed);
		InlineScriptExecutor inlineScriptExecutor = new InlineScriptExecutor(getInlineScriptCode_1627076347791(), inlineScriptContext);
